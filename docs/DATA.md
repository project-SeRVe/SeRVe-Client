...
**출력** (vector_db 디렉토리):
```
~/.serve/vector_db/team-abc123/
├── vectors.npz           # Embeddings + metadata
│   ├── embeddings        # (N, D) float32 array - 모든 타임스텝의 임베딩
│   ├── episode_ids       # (N,) int32 array - 각 벡터가 속한 에피소드 ID
│   └── step_indices      # (N,) int32 array - 에피소드 내 타임스텝 인덱스
├── episodes.json         # 에피소드 메타데이터
├── summary.json          # 벡터 DB 요약 정보
└── index.faiss           # (선택) FAISS L2 인덱스
```
...
| `--write-faiss` | FAISS 인덱스 생성 | False |
...
```
Building vector DB from 5 approved episode(s)...
...
Location: /home/user/.serve/vector_db/team-abc123
Vectors: 750
Episodes: 5
Embedding dim: 49152
✓ FAISS index built
...
```json
{
  "created_at_utc": "2026-03-10T12:34:56.789012+00:00",
  "team_id": "team-abc123",
  "approved_root": "/home/user/.serve/approved/team-abc123",
  "num_episodes": 5,
  "num_vectors": 750,
  "embedding_dim": 49152,
  "embedding_key": "base_image_embeddings",
  "files": ["vectors.npz", "episodes.json", "index.faiss"],
